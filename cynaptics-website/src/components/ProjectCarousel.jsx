...
export default function ProjectCarousel() {
    ...
    (() => {
      ...
      document.querySelectorAll('.Carousel').forEach(($carousel) => {
        ...
        $carousel.addEventListener('wheel', (event) => {
          ...
          const visibleCardIndex = Math.round($carousel.scrollLeft / $cards[0].offsetWidth);
          ...
          const clampedNextCardIndex = Math.max(0, Math.min(nextCardIndex, $cards.length - 1));
  ...
  return (
    <div>
      ...
      <main className="!block !mb-0 !p-0 !h-screen my-2" >


        <section className="Carousel !m-0 !p-0  " id="carousel" tabindex="-1">
          {Projects.map((ele) => {
            return (
              <article key={ele.id} className="Card Card--overlay Card--square group relative transition-all fade-in-out" id={`card-${ele.id}`}>
                <Link
                ...
                >
                  <div className="Card__media">
                    <Image className="Card__image !h-full !w-full group-hover:blur-md" alt="Card image description" placeholder="blur" width="480" height="480" loading="lazy" src={ele.display_image} />
                  </div>
                  <div className='bg-black rounded-t-3xl py-10 px-5 absolute bottom-[-2000px] w-full text-center group-hover:bottom-0 transition-all fade-in-out'>
        ...
        <nav className="Pagination !m-0 !p-0 !mx-10 mt-10 flex justify-center">
          ...
          <div className="Dots">
            {Projects.map((ele) => {
              return (
                <a key={ele.id} href={`#card-${ele.id}`} className="Dot" tabindex="-1"></a>
              )
            })}
