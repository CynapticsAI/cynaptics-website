...
export default function page() {
	useEffect(() => {
		...
		function ballBounce() {
			// platform collision
			if (
				theBall.y + theBall.gravity <= 0 ||
				theBall.y + theBall.gravity + theBall.height >= canvas.height
			) {
				theBall.gravity =
					theBall.gravity *
					-1; /*If it does, change the gravity value to move in the opposite direction*/
				theBall.y += theBall.gravity; // Move theBall
				theBall.x += theBall.speed;
			} else {
				// If the ball doesn’t hit the top or bottom, then move theBall as normal
				theBall.x += theBall.speed;
			...
			if (
				theBall.x <= player1.x + player1.width &&
				theBall.x + theBall.width >= player1.x
			) {
				if (
					theBall.y + theBall.height >= player1.y &&
					theBall.y <= player1.y + player1.height
				) {
			...
			if (
				theBall.x + theBall.width >= player2.x &&
				theBall.x + theBall.width <= player2.x + player2.width
			) {
				if (
					theBall.y + theBall.height >= player2.y &&
					theBall.y <= player2.y + player2.height
				) {
		...
		function loop() {
			/*The loop function will continuously call the draw function and update our screen*/
			if (gameState == 0) {
				init();
			}
			...
			<html>
				...
				<body>
					<div style={{textAlign: "center",paddingTop: "6em"}}>
						<canvas id="canvas" style={{backgroundColor:"#1b1b1b"}}>
							Your browser doesnt support Canvas, try another or update
						</canvas>
					</div>
